...
# convert to NTSC DVD
system("ffmpeg -y -i $vidIn -c:v rawvideo -pix_fmt uyvy422 -target ntsc-dvd -an tempNtscDVD.avi");
# convert to rawvideo format stripping out only the video stream (no DVD packet stream)
sleep 2;
system("ffmpeg -y -i tempNtscDVD.avi -c:v rawvideo -pix_fmt uyvy422 -map 0:v:0 $vidOut");
# cleanup
sleep 2;
system("del /f /q tempNtscDVD.avi");
